...
class OrderViewSet(viewsets.ViewSet):
    ...
    def _send_order_confirmation_emails(self, order, order_items, request):
        """Send order confirmation emails using Brevo HTTP API."""
        import requests
        ...
        # Extract sender email from DEFAULT_FROM_EMAIL (format: "Name <email>")
        ...
        if order.email:
            ...
            try:
                ...
                if response.status_code == 201:
                    ...
                    logger.error(f"Brevo API error (customer): {response.status_code} - {response.text}")
            except Exception as e:
        ...
        if managers.exists():
            ...
            for manager in managers:
                data = {
                    ...
                    "subject": f"New Order #{order.order_number} has been placed",
                    "htmlContent": manager_html,
                    "textContent": manager_plain
                }
                try:
                    response = requests.post(
                        'https://api.brevo.com/v3/smtp/email',
                    ...
                    if response.status_code == 201:
                        ...
                        logger.error(f"Brevo API error for {manager.email}: {response.status_code} - {response.text}")
                except Exception as e:
                    logger.error(f"Manager email failed for {manager.email}: {e}")
