...
class OrderViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        try:
            with transaction.atomic():
                ...
                # Clear cart items (keep cart for reference)
                cart.items.all().delete()
                
                # Send confirmation emails (try, but don't fail order)
                try:
    ...
    def _send_order_confirmation_emails(self, order, order_items, request):
        """Send confirmation emails to customer and order managers."""
        try:
            ...
            logger.error(f"Email sending failed: {str(e)}")
            # Don't re-raise – order is already placed
